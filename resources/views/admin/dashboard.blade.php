<x-admin-layout>
    <div x-data="{ sidebarOpen: false }" class="flex h-screen bg-slate-50 dark:bg-gray-950 font-sans antialiased overflow-hidden">
        ...
        <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'" class="w-72 bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-700 fixed lg:static inset-y-0 left-0 z-50 lg:flex flex-col flex-shrink-0 transform transition-transform duration-300 ease-in-out lg:transform-none">
            <div class="p-8">
            ...
            <nav class="flex-1 px-4 space-y-1 overflow-y-auto custom-scrollbar">
                ...
                <a href="#" @click="sidebarOpen = false" class="group flex items-center px-4 py-3 text-sm font-medium transition-all duration-200 bg-indigo-50 text-indigo-700 dark:bg-indigo-900/20 dark:text-indigo-400 rounded-2xl">
                    ...
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-gray-50 dark:divide-gray-700">
                             @forelse($recentOrders as $item)
                                 <tr class="hover:bg-gray-50/80 dark:hover:bg-gray-700/30 transition-colors cursor-pointer" onclick="this.classList.add('bg-indigo-50', 'dark:bg-indigo-900/20'); setTimeout(() => { window.location.href='{{ $item instanceof \App\Models\Service ? route('admin.services.index', ['highlight' => $item->id]) : route('admin.orders.index', ['highlight' => $item->id]) }}'; }, 100);">
                                     <td class="px-8 py-5 text-sm font-bold text-gray-400 font-mono">
                                         @if($item instanceof \App\Models\Service)
                                             <a href="{{ route('admin.services.index', ['highlight' => $item->id]) }}" class="hover:text-indigo-600">#{{ $item->id }}</a>
                                         @else
                                             <a href="{{ route('admin.orders.index', ['highlight' => $item->id]) }}" class="hover:text-indigo-600">#{{ $item->id }}</a>
                                         @endif
                                     </td>
                                             ...
                                             <div class="flex items-center gap-3">
                                                 <div class="flex items-center gap-2">
                                                     @if($item->client && $item->client->profile_picture)
                                                          <img src="{{ str_starts_with($item->client->profile_picture, 'http') ? $item->client->profile_picture : asset('storage/' . $item->client->profile_picture) }}" alt="{{ $item->client->name }}" class="w-6 h-6 rounded-full object-cover">
                                                     @else
                                                         <div class="w-6 h-6 rounded-full bg-gray-300 dark:bg-gray-600 flex items-center justify-center text-xs text-gray-600 dark:text-gray-400 font-bold">
                                                 ...
                                                 <div class="flex items-center gap-2">
                                                     @if($item->freelancer && $item->freelancer->profile_picture)
                                                         <img src="{{ str_starts_with($item->freelancer->profile_picture, 'http') ? $item->freelancer->profile_picture : asset('storage/' . $item->freelancer->profile_picture) }}" alt="{{ $item->freelancer->name }}" class="w-6 h-6 rounded-full object-cover">
                                                     @else
                                                         <div class="w-6 h-6 rounded-full bg-gray-300 dark:bg-gray-600 flex items-center justify-center text-xs text-gray-600 dark:text-gray-400 font-bold">
                                                             {{ substr($item->freelancer->name ?? 'U', 0, 1) }}
                                                         </div>
