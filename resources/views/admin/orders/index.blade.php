<x-admin-layout>
    <div x-data="{ sidebarOpen: false }" class="flex h-screen bg-slate-50 dark:bg-gray-950 font-sans antialiased overflow-hidden">
        ...
        <main class="flex-1 overflow-y-auto scroll-smooth h-full">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden mb-10 mx-6 lg:mx-10">
                    ...
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-gray-50 dark:divide-gray-700">
                             @forelse($orders as $order)
                                 <tr class="hover:bg-gray-50/80 dark:hover:bg-gray-700/30 transition-colors {{ $order->id == $highlightId ? 'bg-indigo-50 dark:bg-indigo-900/20 ring-2 ring-indigo-500' : '' }}" data-order-id="{{ $order->id }}">
                                    ...
                                    <td class="px-8 py-5 text-right">
                                        <div class="flex items-center justify-end gap-3">
                                            ...
                                            <form action="{{ route('orders.update', $order) }}" method="POST" class="flex items-center gap-2">
                                                @csrf @method('PATCH')
                                                <select name="status" class="text-xs font-semibold py-1 pl-2 pr-8 bg-white dark:bg-gray-900 dark:text-gray-300 border border-gray-200 dark:border-gray-700 rounded-lg focus:ring-indigo-500">
