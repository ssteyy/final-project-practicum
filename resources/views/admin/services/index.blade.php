        ...
        <main class="flex-1 overflow-y-auto scroll-smooth">
            ...
            <div class="p-6 lg:p-10">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <a href="{{ route('admin.services.create') }}" class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-sm font-semibold shadow-md transition inline-block">
                        + Create New
                    </a>
                </div>
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                    ...
                    <div class="bg-gradient-to-br from-emerald-50 to-emerald-100 dark:from-emerald-900/20 dark:to-emerald-800/20 rounded-2xl p-6 border border-emerald-200 dark:border-emerald-800">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-sm font-semibold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider">Published Services</p>
                                <p class="text-3xl font-black text-emerald-700 dark:text-emerald-300 mt-2">{{ $publishedServices }}</p>
                            </div>
                    ...
                    <div class="bg-gradient-to-br from-amber-50 to-amber-100 dark:from-amber-900/20 dark:to-amber-800/20 rounded-2xl p-6 border border-amber-200 dark:border-amber-800">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-sm font-semibold text-amber-600 dark:text-amber-400 uppercase tracking-wider">Draft Services</p>
                                <p class="text-3xl font-black text-amber-700 dark:text-amber-300 mt-2">{{ $draftServices }}</p>
                            </div>
                    ...
                    <div class="bg-gradient-to-br from-red-50 to-red-100 dark:from-red-900/20 dark:to-red-800/20 rounded-2xl p-6 border border-red-200 dark:border-red-800">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-sm font-semibold text-red-600 dark:text-red-400 uppercase tracking-wider">Rejected Services</p>
                                <p class="text-3xl font-black text-red-700 dark:text-red-300 mt-2">{{ $rejectedServices }}</p>
                            </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-gray-50/50 dark:bg-gray-900/50">
                                ...
                                <th class="px-8 py-4 text-xs font-semibold uppercase tracking-wider text-gray-500 text-center">Approval</th>
                                    ...
                                    <td class="px-8 py-5">
                                        <div class="h-14 w-20 rounded-xl overflow-hidden bg-gray-100 dark:bg-gray-700 border border-gray-100 dark:border-gray-600 shadow-sm">
                                            @if($service->image_path)
                                                <img src="{{ asset('storage/' . $service->image_path) }}" alt="{{ $service->title }}" class="h-full w-full object-cover">
                                            @elseif($service->image_url)
                                                <img src="{{ $service->image_url }}" alt="{{ $service->title }}" class="h-full w-full object-cover">
                                            @else
                                                <div class="flex items-center justify-center h-full w-full text-gray-400 bg-gray-50 dark:bg-gray-800">
                                        ...
                                        <div class="flex items-center justify-center gap-2">
                                            @if($service->status !== 'published')
                                                <form action="{{ route('admin.services.approve', $service) }}" method="POST">
                                                    @csrf @method('PATCH')
                                                    <button type="submit" class="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded-lg transition shadow-sm shadow-emerald-200 dark:shadow-none">
                                                        Approve
                                                    </button>
                                                </form>
                                            @endif
                                            ...
                                            @if($service->status !== 'rejected')
                                                <form action="{{ route('admin.services.reject', $service) }}" method="POST">
                                                    @csrf @method('PATCH')
                                                    <button type="submit" class="px-3 py-1.5 bg-white dark:bg-gray-800 border border-red-200 dark:border-red-900/50 text-red-600 dark:text-red-400 text-xs font-bold rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 transition">
                                                        Reject
                                                    </button>
                                                </form>
                                            @endif
